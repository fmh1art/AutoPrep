...
class ColEDataset(BaseDataset):
    def __init__(self, dataset_name:str, sample_limit=4000):
        super(ColEDataset, self).__init__(dataset_name)
    ...
    def load_data(self, data_path=None):
        ...
        if data_path == None:
            data_path = f'{DATA_PATH}/ColE/{self.dataset_name}/'
        else:
            data_path = f'{data_path}/ColE/{self.dataset_name}/'
        ...
        for i in range(file_num*2):
            ...
            file_name = f'{tmp}_ColE_inses_{i}.jsonl'
            ...
            for line_id, d in enumerate(ds):
                ...
                celle_data = ColEData(self.dataset_name, tbl=d['table'], clue=d['clue'], column=d['column'], offset=d['offset'],
                                       data_type=d['type'], task_type='ColE', id=f'{file_name}:{line_id}')
