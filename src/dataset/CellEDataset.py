...
class CellEDataset(BaseDataset):
    def __init__(self, dataset_name:str, sample_limit=4000):
        super(CellEDataset, self).__init__(dataset_name)
    ...
    def load_data(self, data_path=None):
        ...
        if data_path == None:
            data_path = f'{DATA_PATH}/CellE/{self.dataset_name}/'
        else:
            data_path = f'{data_path}/CellE/{self.dataset_name}/'
        ...
        for i in range(file_num*2):
            ...
            file_name = f'{tmp}_CellE_inses_{i}.jsonl'
            ...
            for line_id, d in enumerate(ds):
                ...
                if len(d['table']) > 24 or len(d['table'][1]) > 10:
                    continue
                celle_data = CellEData(self.dataset_name, tbl=d['table'], clue=d['clue'], cell=d['cell'], offset=d['offset'],
                                       data_type=d['type'], task_type='CellE', id=f'{file_name}:{line_id}')
                loaded_ds.append(celle_data)
