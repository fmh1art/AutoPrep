...
class RowEDataset(BaseDataset):
    def __init__(self, dataset_name:str, sample_limit=4000):
        super(RowEDataset, self).__init__(dataset_name)
    ...
    def load_data(self, data_path=None):
        ...
        if data_path == None:
            data_path = f'{DATA_PATH}/RowE/{self.dataset_name}/'
        else:
            data_path = f'{data_path}/RowE/{self.dataset_name}/'
        ...
        for i in range(file_num*2):
            ...
            file_name = f'{tmp}_RowE_inses_{i}.jsonl'
            ...
            for line_id, d in enumerate(ds):
                ...
                rowe_data = RowEData(self.dataset_name, d['table'], d['clue'], d['row'], d['offset'], d['type'], id=f'{file_name}:{line_id}')
                loaded_ds.append(rowe_data)
