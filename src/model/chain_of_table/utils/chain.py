...
def _conduct_single_solver_mp_core(arg):
    ...
    # print(idx, sample, llm, solver_func, kwargs)
    try:
        table_info = get_table_info(
            sample,
            skip_op=kwargs.get("skip_op", []),
        ...
        # print("after table info",table_info)
        proc_sample = solver_func(sample, table_info, llm, **kwargs)
        return idx, proc_sample
    except Exception as e:
        print("_conduct_single_solver_mp_core")
        print(f"Error in {idx}-th sample: {e}")
        return idx, None
...
plan_add_column_demo = """If the table does not have the needed column to answer the question, we use f_add_column() to add a new column for it. For example,
/*
...
*/
Question: how many atheletes are from japan? 
Function: f_add_column(country of athlete)
Explanation: The question asks about the number of athletes from japan. We need to known the country of each athlete. There is no column of the country of athletes. We add a column "country of athlete"."""
...
*/
Question: what is the population of momasa?
Function: f_select_column(county, population)
Explanation: The question asks the population of mombasa. We need to know the county and its population. We select the column "county" and column "population"."""
...
*/
Question: which engine in the jeep grand cherokee had the third lowest numbered displacement?
Function: f_select_row(row 1, row 4, row 5)
Explanation: The question asks the engine had third lowest numbered displacement. We need to know the first three low numbered displacement and all rows that power is om647 diesel i5. We select the row 1, row 4, row 5."""
...
Explanation: The question asks the number of districts that are democratic. We need to know the number of dem in the table. We group the rows according to column "party"."""
...
*/
Question: was cd numancia placed in the last position?
Function: f_sort_column(position)
Explanation: The question asks the position of cd numancia. We need to know the order of position from last to front. We sort the rows according to column "position"."""
...
*/
Question: which year is the last year where this team was a part of the usl a-league?
...
*/
Question: how many atheletes are from India?
...
*/
Question: when is the competition with highest points scored is played on?
...
def dynamic_chain_exec_with_cache_for_loop(
    ...
    def _func(idx):
        ...
        if os.path.exists(cache_path):
            ...
            _, proc_sample, log = json.load(open(cache_path, "r"))
        else:
            ...
            json.dump((sample, proc_sample, log), open(cache_path, "w"))
        result_samples[idx] = proc_sample
...
def _dynamic_chain_exec_with_cache_mp_core(arg):
    ...
    try:
        ...
        print("_dynamic_chain_exec_with_cache_mp_core")
