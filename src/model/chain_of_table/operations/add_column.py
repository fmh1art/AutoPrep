...
def add_column_build_prompt(table_text, question, table_caption=None, num_rows=100):
...
def add_column_func(
    ...
    cleaned_question = sample["cleaned_statement"]
    cleaned_question = re.sub(r"\d+", "_", cleaned_question)

    prompt = "" + add_column_demo.rstrip() + "\n\n"
    prompt += add_column_build_prompt(
        table_text, cleaned_question, table_caption=table_caption, num_rows=3
    )
