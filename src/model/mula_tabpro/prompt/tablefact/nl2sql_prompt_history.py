...
SQL: SELECT (SELECT `new_entries_this_round` FROM w WHERE round = 'third round') IS NULL
...
3 example rows:
SELECT * FROM w LIMIT 3;
row_id	date	visiting_team	final_score	host_team	stadium
...
SQL: SELECT (SELECT (SELECT `host_team` FROM w WHERE date = 'new year eve') = 'new york giant') AND (SELECT (SELECT `host_team` FROM w WHERE date = 'new year's day') = 'st louis ram')
...
3 example rows:
SELECT * FROM w LIMIT 3;
row_id	call sign	frequency	city_of_license	facility_id	erp_power_w	height m ( ft )	class
